...
public class OmfComentWeakExternals extends OmfComent {
	...
	public record WeakExternalPair(int weakExternalIndex, int defaultResolutionIndex) {
		/**
		 * Validates the weak external indices.
		 */
		public WeakExternalPair {
			if (weakExternalIndex < 0 || weakExternalIndex > OmfUtils.MAX_INDEX_VALUE) {
				throw new IllegalArgumentException(
						"WEAK_EXTERNALS weak external index out of range: " + weakExternalIndex);
			}

			if (defaultResolutionIndex < 0 || defaultResolutionIndex > OmfUtils.MAX_INDEX_VALUE) {
				throw new IllegalArgumentException(
						"WEAK_EXTERNALS default resolution index out of range: " + defaultResolutionIndex);
			}
		}
	}
	...
	public OmfComentWeakExternals(ByteInputStream bis) throws IOException {
		...
		this.pairs = List.copyOf(pairList);
	}
